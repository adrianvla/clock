...
export default class DigitalTimeBigWidget extends Widget {
    constructor(x,y) {
        super(x,y);
        this.hours = new Text("", Fonts['5x5']);
        this.minutes = new Text("", Fonts['5x5']);
        this.dots = new Text(":");
    }
    ...
    render(x, y, dt) {
        ...
        this.minutes.draw(x+15, y+0, 0xFFFFFF);

        const seconds = now / 1000;
        const flicker = (Math.sin(2 * Math.PI * seconds) * 0.375) + 0.625;
        this.dots.draw(x+12, y+0, dimHex(0xFFFFFF, flicker));
    }
    getBBox(){
        return {width: 26, height:5};
    }
}